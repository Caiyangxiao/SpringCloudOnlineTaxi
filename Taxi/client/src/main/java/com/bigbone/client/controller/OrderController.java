...
public class OrderController {
    ...
    @PostMapping("save/{deid}/{departure}/{destination}/{name}/{typeValue}")
    @ResponseBody
    public Msg save(@PathVariable("deid")int deid, @PathVariable("departure") String departure,
                    @PathVariable("destination") String destination,
                    @PathVariable("name") String name, @PathVariable("typeValue") String typeValue, HttpSession session){
        Driver driver = (Driver) session.getAttribute("driver");
//        Demand demand = new Demand(deid, departure, destination, name);
        Order order = new Order();
        order.setDeparture(departure).setDestination(destination)
                .setPassenger(name).setDriver(driver.getUsername()).setType(typeValue)
        .setPrice(Math.abs(Integer.parseInt(destination,16) - Integer.parseInt(departure,16)));
