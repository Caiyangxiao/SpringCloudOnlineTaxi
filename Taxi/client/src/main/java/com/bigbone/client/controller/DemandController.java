...
public class DemandController {
    ...
    public Result getPassengerDemand(@RequestParam("page") int page, @RequestParam("limit") int limit,
        ...
        return new Result(0, "", demands.size(), demands);
    }
    ...
    @GetMapping("getDriverShareDemand")
    @ResponseBody
    public Result getDriverShareDemand(@RequestParam("page") int page, @RequestParam("limit") int limit,
                                  HttpSession session){
        Driver driver = (Driver) session.getAttribute("driver");
        List<Demand> demands = demandFeign.getDriverShareDemand(driver.getAddress(),(page - 1) * limit, limit);
        return new Result(0, "", demands.size(), demands);
    }
    ...
    public String saveShare(Demand demand, HttpSession session){
        Passenger passenger = (Passenger) session.getAttribute("passenger");
        demand.setName(passenger.getUsername()).setDeparture(passenger.getAddress());
        demand.setType("share");
        Result vo = demandFeign.saveShare(demand);
        return "redirect:/demand_manage.html";
    }
