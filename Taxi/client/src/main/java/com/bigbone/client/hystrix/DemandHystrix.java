...
public class DemandHystrix implements FallbackFactory {
    ...
    public Object create(Throwable throwable) {
        return new DemandFeign() {
            ...
            @Override
            public List<Demand> getDriverShareDemand(String address, int start, int row) {
                return null;
            }
            ...
            @Override
            public Result saveShare(Demand demand) {
                return new Result().setCode(499).setMsg("服务降级，现在不允许添加拼车需求，请稍后再试！");
            }
