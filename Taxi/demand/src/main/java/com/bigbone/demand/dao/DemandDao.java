...
public interface DemandDao {
    ...
    @Select("select * from t_demand where departure = #{address} and type='own' limit #{start}, #{row}  ")
    List<Demand> findAll1(@Param("address") String address, @Param("start") int start, @Param("row") int row);

    @Select("select * from t_demand where destination = #{address} and type='share' limit #{start}, #{row} ")
    List<Demand> findAll2(@Param("address") String address, @Param("start") int start, @Param("row") int row);
...
//    Demand findById(Integer id);
    ...
    @Insert("insert into t_demand(departure, destination, name,type) values(#{departure}, #{destination}, #{name},#{type})")
    void saveShare(Demand demand);
