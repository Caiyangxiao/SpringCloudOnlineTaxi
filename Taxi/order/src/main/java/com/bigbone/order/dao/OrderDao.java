...
public interface OrderDao {
    @Insert("insert into t_order(departure,destination,state,passenger,driver,price,type) " +
            "values(#{departure}, #{destination}, 0, #{passenger}, #{driver}, #{price}, #{type})")
    void save(Order order);

    @Select("select id, departure, destination, state, passenger, driver, price,type from t_order" +
